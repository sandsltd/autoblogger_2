...
async function downloadImage(imageUrl, filename, imagesPath) {
  try {
...
async function generateBlogImage(topic, title, config, openai) {
  ...
  try {
    ...
    if (response.data && response.data[0]) {
      ...
      const localImagePath = await downloadImage(imageUrl, filename, imagesPath);
...
async function saveBlogPost(post, config) {
  ...
  const frontmatter = {
    ...
    canonicalUrl: `${config.business.website}/blog/${slug}`
  };
